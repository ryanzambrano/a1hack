// Product detail / configuration page for variant E "Nordisk".
...
function updateCartBadge() {
  ...
  if (link) {
    if (count === 0) {
      link.setAttribute("aria-label", "Handlekurv, tom");
    } else if (count === 1) {
      link.setAttribute("aria-label", "Handlekurv, 1 vare");
    } else {
      link.setAttribute("aria-label", "Handlekurv, " + count + " varer");
    }
  }
}
...
async function loadShopHeader() {
  try {
    ...
    // Keep the fallback name in the static markup.
  }
}
...
function renderProduct(product) {
  ...
  // Option groups: single-select radio cards, defaults preselected.
  ...
  groups.forEach((group, groupIndex) => {
    ...
    const legend = el("legend", "", group.name);
    fs.append(legend);
    const list = el("div", "option-list");

    // Size-like groups (deltas differ between options) show absolute
    // prices (base + delta); other groups show "+ kr X" on non-zero deltas.
    const differing = new Set(group.options.map((o) => o.priceDeltaCents)).size > 1;
  ...
  if (product.canHaveCakeText) {
    ...
    const lbl = el("label", "", "Tekst på kaken");
  ...
  const qtyLabel = el("label", "", "Antall");
  ...
  totalRow.append(el("span", "", "Totalt"), totalValue);
  ...
  addBtn.addEventListener("click", () => {
    ...
    if (!result.ok) {
      ...
      panel.append(el("p", "feedback-text",
        "Handlekurven er full. Du kan ha maks " + MAX_CART_LINES + " ulike varer i en bestilling."));
      feedback.append(panel);
      return;
    }

    const panel = el("div", "feedback-panel feedback-ok");
    panel.append(el("h2", "feedback-title", "Lagt i handlekurven"));
    const desc = qty === 1
      ? product.name + " er lagt i handlekurven."
      : qty + " stk " + product.name + " er lagt i handlekurven.";
    panel.append(el("p", "feedback-text", desc));
    const actions = el("div", "feedback-actions");
    const toCart = el("a", "btn btn-primary", "Til handlekurven");
    toCart.href = "./cart.html";
    const keepShopping = el("a", "btn", "Fortsett å handle");
