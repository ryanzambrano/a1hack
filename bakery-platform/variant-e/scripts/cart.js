// Cart page script for variant E (Nordisk).
...
function renderError() {
  ...
  card.appendChild(el("p", null, "Vi fikk ikke lastet handlekurven. Sjekk nettverket og prøv igjen."));
...
function renderLine(cart, productsById, line) {
  ...
  if (details.length) {
    info.appendChild(el("p", "line-meta", details.map((d) => d.group + ": " + d.value).join(" · ")));
  }
  if (line.cakeText && product.canHaveCakeText) {
    info.appendChild(el("p", "line-meta", "Kaketekst: «" + line.cakeText + "»"));
  }
...
function render(cart, productsById) {
  ...
  const listCard = el("section", "card cart-card");
...
async function init() {
  root.textContent = "";
  root.appendChild(el("p", "muted", "Laster handlekurven..."));
